...
# Tokenisation is a foundational step in many NLP tasks. Tokenising text is the process of splitting a piece of text into words, symbols, punctuation, spaces,
# and other elements, thereby creating “tokens”. A naive way to do this is to simply split the string on white space:
doc.text.split()

'''Output: ['Build', 'your', 'data', 'science', 'skills', 'to', 'launch', 'an', 'in-demand,'
, 'valuable', 'career', 'in', 'six', 'months.']'''

# On the surface, this looks fine. But, note that a) it disregards the punctuation and Put differently, it is naive, it fails to recognise elements of the text that help
...
# Here we access the each token’s .orth_ method, which returns a string representation of the token rather than a spaCy token object, this might not always be desirable,
# but worth noting. SpaCy recognises punctuation and is able to split these punctuation tokens from word tokens. Many of spaCy’s token methods offer both string 
# and integer representations of processed text – methods with an underscore suffix return strings, methods without an underscore suffix return integers. For example:
...
...
'''
# If you want to avoid returning tokens that are punctuations or white space, spaCy provides convienient methods for this
...
# word often have the same root meaning. For example, practice, practised and practising all essentially refer to the same thing. It is often desirable to standardise 
# words with similar meaning to their base form. With spaCy we can access each word’s base form with a token’s .lemma_ method:
...
# Why is this useful? An immediate use case is in machine learning, specifically text classification. Lemmatising the text prior to, for example, creating a
# “bag-of-words” avoids word duplication and, therefore, allows for the model to build a clearer picture of patterns of word usage across multiple documents.


# Entity recognition

# Entity recognition is the process of classifying named entities found in a text into pre-defined categories, such as persons, places, organisations, dates, etc. 
# spaCy uses a statistical model to classify a broad range of entities, including persons, events, works-of-art and nationalities / religions 
# (see the documentation for more information https://spacy.io/docs/usage/entity-recognition).

# For example, let’s take the first two sentences from Priyanka Chopra's wikipedia entry. We will parse this text, then access the identified entities using 
# the doc object’s .ents method. With this method called on the doc we can access additional token methods, specifically .label_ and .label:
